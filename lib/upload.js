...
'use strict';
  ...
  function read(){
    ...
    basePath = '/Users/jimmytsao/Documents/Code/projects/projectfp/Server/images/lotto/';

    imagePath = basePath + 'lottery' + formatNumber(imageNumber++) + '.jpg';
    ...
    tesseract.process(imagePath, function(err, text) {
      ...
      realValue = values.values['lottery'+formatNumber(imageNumber-1)];
      readValue = text.trim()
      if(err) {
        res.send(500);
      } else {
        console.log('lottery'+formatNumber(imageNumber-1)+': ',readValue === realValue, ' :: ', realValue, '   ', readValue);
        if (imageNumber <= 50) {
          read();
        }
      }
